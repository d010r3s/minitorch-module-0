...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    s = sigmoid(a)
    assert 0.0 <= s <= 1.0
    assert math.isclose(1.0 - s, sigmoid(-a), rel_tol=0.0, abs_tol=1e-6)
...
def test_symmetric(x: float, y: float) -> None:
    ...
    """
    # TODO: Implement for Task 0.2.
    assert math.isclose(mul(x, y), mul(y, x), rel_tol=1e-6, abs_tol=1e-6)
...
def test_distribute(z: float, x: float, y: float) -> None:
    ...
    """
    # TODO: Implement for Task 0.2.
    assert math.isclose(mul(z, add(x, y)), add(mul(z, x), mul(z, y)), rel_tol=1e-6, abs_tol=1e-6)
...
def test_other(x: float) -> None:
    ...
    # TODO: Implement for Task 0.2.
    assert math.isclose(neg(neg(x)), x, abs_tol=1e-12)
    assert math.isclose(add(x, neg(x)), 0.0, abs_tol=1e-12)
